...
class Gpio final : public sjsu::Gpio
{
 public:
  ...
  static lpc40xx::LPC_GPIO_TypeDef ** GpioRegister(int port_index)
  {
    ...
    else if constexpr (IsPlatform(sjsu::build::Platform::lpc18xx))
    {
      static lpc40xx::LPC_GPIO_TypeDef * port[] = {
  ...
  constexpr Gpio(uint8_t port_number,
      ...
      : klpc18xxPin(port_number, pin_number),
  ...
  {
    /// Pin function is zero fall pins on the LPC40xx and lpc18xx.
    constexpr uint8_t kGpioFunction = 0;
    pin_obj_->SetPinFunction(kGpioFunction);

    if (direction == Direction::kInput)
    {
      LPC_GPIO->DIR[port_] = bit::Clear(LPC_GPIO->DIR[port_], pin_);
    }
    else
    {
      LPC_GPIO->DIR[port_] = bit::Set(LPC_GPIO->DIR[port_], pin_);
    }
  }
  void Set(State output = kHigh) const override
  {
    ...
    {
      LPC_GPIO->SET[port_] = (1 << pin_);
    }
    else
    {
      LPC_GPIO->CLR[port_] = (1 << pin_);
    }
  }
  void Toggle() const override
  {
    LPC_GPIO->NOT[port_] = (1 << pin_);
  }
 ...
 private:
  ...
  const sjsu::lpc17xx::Pin klpc18xxPin;
